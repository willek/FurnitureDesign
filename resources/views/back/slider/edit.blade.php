...
<div class="block">
  ...
  <form id="form-data" action="{{ route('back.updated_slider', $slider->id) }}" method="post" enctype="multipart/form-data" class="form-horizontal form-bordered">
    {!! csrf_field() !!}
    <div class="form-group">
      <label class="col-md-2 control-label" for="example-name">Name</label>
      <div class="col-md-8">
        <input type="text" id="example-name" name="name" class="form-control" placeholder="Name" required value="{{ $slider->name }}">
      </div>
    </div>
    <div class="form-group">
      <label class="col-md-2 control-label" for="example-file">Image</label>
      <div class="col-md-8">
          <a href="{{ $slider->imagedir }}" data-toggle="lightbox-image">
              <img src="{{ $slider->imagedir }}" class="img-responsive img-rounded img-edit">
          </a>
    ...
    <div class="form-group">
      ...
      <div class="col-md-8">
        <label class="switch switch-primary"><input name="status" type="checkbox" {{ ($slider->status == 1) ? 'checked' : '' }}><span></span></label>
      </div>
    </div>
